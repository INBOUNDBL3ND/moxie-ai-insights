# July 2026 MOXIE Report Generation Spec

You are generating monthly client dashboard reports. For each assigned client you will WRITE two files:
1. `clients/<NUM>/july-2026.html` (new report)
2. Update `clients/<NUM>/index.html` (prepend a July card)
...
- `data/july_inputs.json[<NUM>]` → `{name, status, audience, pills, has_july_paid, paid_spend, contextNotes}`
- `data/july_perf.json[<NUM>]` → `{name, has_july_paid, paid_spend, sessions, rows:[{platform,budget,spend,impr,reach,clicks,ctr}]}` (July metrics). **`sessions`** = July website sessions (use this for the "Website Sessions" stat and the Website Traffic card; the Website-Traffic `rows` entry has impr=0, do NOT use it for sessions).
- `data/forward_look_overrides.json` → `forward_look` (use forward-look card instead of a reco) and `scope` (hard framing constraints)
- `clients/<NUM>/june-2026.html` → prior report. Carry forward ONLY recurring services; copy template structure.
- **Slack**: load `mcp__claude_ai_Slack__slack_search_channels` + `slack_read_channel` via ToolSearch. Find the channel (`slack_search_channels` query = the 7-digit `<NUM>`), then `slack_read_channel` with `oldest`/`latest` bounding **July 1–31 2026** (oldest ts `1782950400`, latest ts `1785628799`). Read July activity ONLY. **July 2026 epoch bounds: oldest=`1782950400` latest=`1785628799`** (July 1 00:00 UTC – July 31 23:59 UTC 2026). Do NOT use June or 2025 bounds.

## Hard gates (validate.py enforces all of these — 0 failures required)
1. **PAID vs ORGANIC** (the #1 rule): The Analysis may use paid/advertising framing ONLY if `has_july_paid` is true (or scope override allows). If `has_july_paid` is false → ORGANIC: never write "paid side/channels/media/mix", "ad spend", "campaigns", "retargeting/remarketing", "impressions/clicks/CTR", "advertising", "boosted", or name an ad platform in the Analysis. Organic clients talk about: the website, hosting, SEO, organic social content, email, the audience, the brand, readiness. (The MOXIE Recommends card MAY name a paid service — that's a recommendation, allowed.)
2. **MOXIE Recommends gate**: A service may appear in "Add X" ONLY if it is (a) NOT in the client's pills, (b) NOT a platform row in `july_perf`, (c) NOT declined in June Slack. If none fits, OR the client is in `forward_look` overrides → use the forward-look card (h4 title + p), no "Add X". Never recommend something we already run.
...
Only JULY activities from Slack. One-time items (meetings, builds, specific fixes) appear once, in July only — do NOT repeat June's one-time items. Recurring service lines (Social Media Posts, Email Newsletter, Website Management, SEO, Ads management, Hosting/maintenance) may appear each month. If July Slack is thin, list the recurring services that apply per the pills.
...
## TEMPLATE A — paid client (has_july_paid true)
Use the full template: Analysis → Performance Overview (Ad Impressions = sum of ad-platform impressions; Ad Clicks = sum of ad clicks; Website Sessions = the `sessions` field) → Platform Breakdown (one card per ad platform with July metrics from july_perf, plus a Website Traffic card) → What We Did → Active Services pills → MOXIE Recommends. Copy the exact HTML structure from the client's `june-2026.html`, swap month to July, replace numbers with July metrics, rewrite Analysis/Activity/Recommends.
...
## TEMPLATE B — organic / hosting (has_july_paid false)
...
<head>
  ...
  <title><CLIENT NAME> &mdash; July 2026 | MOXIE AI Insights</title>
...
<body>
  <div class="container">
    <div class="report-header">
      ...
      <div class="report-month">July 2026</div>
      <div class="client-name"><CLIENT NAME></div>
    </div>
    <div class="breadcrumb"><a href="/">Home</a> &rsaquo; <a href="/clients/<NUM>/"><CLIENT NAME></a> &rsaquo; July 2026</div>
...
## index.html update
Prepend this card immediately after `<div class="reports-grid">` (so July is first):
```html
            <div class="report-card">
        <div class="month">July</div>
        <div class="year-badge">2026</div>
        <a href='/clients/<NUM>/july-2026'>View Report &rarr;</a>
      </div>
```

## Pills
Use the client's `pills` from july_inputs.json verbatim as the Active Services pills. (Exception: Epoxy 2010001 scope override — see overrides; still list contracted pills but body stays website-only.)
